...
//	@Query("SELECT i FROM Interviews i WHERE (i.intervieweeId = :intervieweeId) AND i.date = :date AND ((i.startTime >= :startTime AND i.startTime < :endTime) OR (i.endTime > :startTime AND i.endTime <= :endTime) OR (i.startTime>=:startTime AND i.endTime<=:endTime))")
//	List<Interviews> findByInterview(int intervieweeId, LocalDate date, LocalTime startTime, LocalTime endTime);
	...
	@Query(value = "SELECT * FROM interviews i  WHERE i.meeting_status =:meetingStatus AND (i.batch = :batch OR :batch is null) order by i.date,i.start_time", nativeQuery = true)
	...
	@Query("select i FROM Interviews i where i.interviewerId =?1 AND i.date=?2 AND i.startTime>=?2 AND i.endTime<= ?3")
	List<Interviews> findInterviewsByDateAndTimeForAnAdmin(Integer adminId, LocalDate date, LocalTime startTime, LocalTime endTime);

	
	@Query("select i FROM Interviews i where i.batch=?1 OR i.meetingStatus=?2 OR i.category =?3 OR i.date =?4 OR i.title=?5")
	List<Interviews> findAllByFilters(String batch, String meetingStatus,String category, LocalDate date,String title);
