...
//	delete 
	...
	@DeleteMapping("/delete/{adminid}/{localdate}")
	public ResponseEntity<String> deleteSlotByDate(@PathVariable Integer adminid, @PathVariable LocalDate localdate) {
		String dlt = slotService.deleteSlotByDate(adminid, localdate);
		return new ResponseEntity<>(dlt,HttpStatus.OK);
	}

	@PreAuthorize("hasRole('ADMIN')")
	@PostMapping("/create-slots")
	public ResponseEntity<String> createOneOnOneSlots(@RequestBody OneOnOne oneOnOne) throws Exception {
