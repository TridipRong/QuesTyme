...
import com.QuesTyme.dto.interviewDtoOM;
import com.QuesTyme.entity.Interviews;
import com.QuesTyme.entity.InterviewsOM;
...
import com.QuesTyme.exceptions.SlotException;
import com.QuesTyme.repository.InterviewOMRepository;
import com.QuesTyme.entity.Slot;
import com.QuesTyme.entity.User;
import com.QuesTyme.exceptions.InterviewException;
...
public class InterviewService {
	...
	@Autowired
	private InterviewOMRepository interviewOMrepo;
	...
	public InterviewsOM createOM(interviewDtoOM interviewom) throws InterviewException {				
		
		
		Map<String, User> user = userMap();
		User interviewer = user.get(interviewom.getInterviewerEmail());
		List<User> interviewees = new ArrayList<>();
		List<String> intervieweeEmails = interviewom.getIntervieweeEmail();
		
		List<Slot> slots = slotService.getAllSlotByStartTime(interviewom.getStartTime());
		if(slots.size()!=0) {
			throw new SlotException("Slot found with this start time, Schedule the interview when no slot found");
		}
//		List<Interviews> interviews = interviewrepo.findByInterview(interviewom.getDate(),
//				interviewom.getStartTime(), interviewom.getEndTime(), interviewer.getId());
//		if (!interviews.isEmpty()) {
//			throw new InterviewException("Interviewer is already booked for the given date and time");
//		}
		

		for (String email : intervieweeEmails) {
			User interviewee = user.get(email);
			interviewees.add(interviewee);
		}
		InterviewsOM interview = InterviewMapper.mapToInterview(interviewom, interviewees, interviewer);
		InterviewsOM savedInterview = interviewOMrepo.save(interview);
...
//		interviewee.getName()	+
				...
				+ "\n\nBest regards,\nThe Interview Team";
//		emailService.sendEmail(interviewom.getInterviewerEmail(), interviewerSubject, interviewerBody);
...
//		String intervieweeBody = "Dear " +
////		interviewees.getName() +
//		",\n\nYour interview with " + interviewer.getName()
//				+ " has been scheduled for " + interview.getDate() + " from " + interview.getStartTime() + " to "
//				+ interview.getEndTime() + ".\n\nPlease use the following link to join the meeting: "
//				+ interview.getMeetingLink() + "\n\n Or you can visit the interview platform"
//				+ "\n\nBest regards,\nThe Interview Team";
//		emailService.sendEmail(interviewom.getIntervieweeEmail(), intervieweeSubject, intervieweeBody);
	...
	public Interviews createInterview(InterviewDto interviewdto) throws InterviewException {
		...
		List<Interviews> existingInterviews = interviewrepo.findByInterview(interviewee.getId(), interviewdto.getDate(),interviewdto.getStartTime(), interviewdto.getEndTime(), interviewer.getId());
		
		if (!existingInterviews.isEmpty()) {
			throw new InterviewException("Interviewee is already booked for the given date and time");
		}
		
		List<Slot> existingSlots = slotRepository.findSlotByDateAndTime(interviewer.getId(), interviewdto.getDate(), interviewdto.getStartTime() , interviewdto.getEndTime()); 
		...
		// Send an email to the interviewer to notify them about the scheduled interview.
		String interviewerSubject = "Interview Scheduled Successfully";
		...
		// Send an email to the interviewee to notify them about the scheduled interview.
		String intervieweeSubject = "Interview Scheduled Successfully";
		...
		emailService.sendEmail(interviewee.getEmail(),interviewerSubject,interviewerBody);
	 ...
	 * 
	 * @param interviewId and feedback in the form of string 
	 * @return the feedback will added to interview
	 * @throws InterviewException if there is an error creating the interview
	 */
	public String addInterviewFeedback(Integer interviewId, String feedback) throws InterviewException {
		String msg;
		  Optional<Interviews> optionalInterviews = interviewrepo.findById(interviewId);
		    if (optionalInterviews.isPresent()) {
		        Interviews interviews = optionalInterviews.get();
		        interviews.setAdminFeedback(feedback);
	 ...
	 * Student Notes.
	 * 
	 * @param interviewId and Student Notes in the form of string 
	 * @return the student note will added to interview
	 * @throws InterviewException if there is an error creating the interview
	 */
	public String addStudentNotes(Integer interviewId, String notes) throws InterviewException{
		String msg;
		  Optional<Interviews> optionalInterviews = interviewrepo.findById(interviewId);
		    if (optionalInterviews.isPresent()) {
		        Interviews interviews = optionalInterviews.get();
		        interviews.setStudentsNotes(notes);
		        interviewrepo.save(interviews);
...
//	    Map<String, User> user = userMap();
	 ...
	 // Creating thread pool with 10 threads
	    ...
	    for (int i = 1; i < rows.size(); i++) {
	        ...
	        String interviewerBody = "Dear " + interviewer.getName() + ",\n\nYour interview with "
	                ...
	                + ".\n\nPlease use the following link to join the meeting: " + interview.getMeetingLink()
	                + "\n\n Or you can visit the interview platform" + "\n\nBest regards,\nThe Interview Team";
	        Runnable sendInterviewerEmailTask = () -> emailService.sendEmail(row[0], interviewerSubject, interviewerBody);
	        ...
	        String intervieweeBody = "Dear " + interviewee.getName() + ",\n\nYour interview with "
	                ...
	                + ".\n\nPlease use the following link to join the meeting: " + interview.getMeetingLink()
	                + "\n\n Or you can visit the interview platform" + "\n\nBest regards,\nThe Interview Team";
	        Runnable sendIntervieweeEmailTask = () -> emailService.sendEmail(row[1], intervieweeSubject, intervieweeBody);
	...
	public Interviews updateMeetingStatus(int interviewId, String status, int userId) throws InterviewException {
		...
		if (interview != null && status.equals("S")) {
			if (currentDateTime.isEqual(startDateTime.minusMinutes(5))
					|| currentDateTime.isAfter(startDateTime.minusMinutes(5))
							&& (currentDateTime.isBefore(startDateTime) || currentDateTime.isEqual(startDateTime))) {
	 ...
	 * @throws InterviewException if there is an error retrieving the interview data
	 */
	public List<InterviewResponseDto> filterInterviews(String batch, String meetingStatus,String category,LocalDate date,String title) throws InterviewException {
		System.out.println(3);
		
		List<Interviews> interviews = interviewrepo.findAllByFilters(batch,meetingStatus,category,date,title);
